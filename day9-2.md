...
pd.read_csv("C:/Users/ydo16/Desktop/code/data/kimchi_test.csv", encoding='cp949')
...
kimchi.groupby(by=None,      # 그룹핑 할 컬럼(기준)
               Axis = 0,     # 그룹핑 연산 방향
               Level = None) # 멀티 인덱스 경우, 특정 레벨의 값을 그룹핑 컬럼으로 사용
...
# 예제) 제품별, 판매처별(김치별) 수량 판매금액 총 합, 평균
...
kimchi_2.groupby(level='판매처').sum()
>>>
판매처
대형마트    4691377
백화점     1735685
